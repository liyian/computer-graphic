...
struct DiffuseBRDF : public BRDF {
	...
	void sample(const Vec &n, const Vec &o, Vec &i, double &pdf) const {
		// FIXME
		i = uniformRandomPSA(n);// i= ωi-->light direction
		pdf = i.dot(n) / PI; //
	}
...
Vec receivedRadiance(const Ray &r, int depth, bool flag) {
	...
	*/
	...
	if (rng() < p) {
		Vec i;
		double pdf;
		brdf.sample(n, o, i, pdf);//contains Vec i = uniformRandomPSA(n);
		Ray y(x, i);
		return rad + brdf.eval(n, o, i).mult(receivedRadiance(y, depth + 1, flag))*(PI / p);
